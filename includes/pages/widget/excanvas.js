...
if (!window.CanvasRenderingContext2D) {
  (function () {
    ...
    function getObjectHtml(fn) {
      return (
        '<div><object type="application/x-silverlight" >' +
    ...
    contextPrototype.drawImage = function (image, var_args) {
      ...
      if (arguments.length == 9) {
        ...
        var clipRect = create(this, '<RectangleGeometry Rect="%1,%2,%3,%4"/>', [
          sx,
          sy,
          sw,
          sh,
    ...
    contextPrototype.stroke = function () {
      ...
      path.strokeEndLineCap = path.strokeStartLineCap = processLineCap(
        this.lineCap
      );
    };
    ...
    contextPrototype.clip = function () {
      if (this.currentPath_.length) {
        ...
        var canvas = create(
          this,
          '<Canvas Width="%1" Height="%2" Clip="%3"/>',
    ...
    CanvasMega_.prototype.createStops_ = function (ctx, brushObj, colors) {
      ...
      for (var i = 0, c; (c = colors[i]); i++) {
        ...
        gradientStopCollection.add(
          create(ctx, '<MegaStop Color="%1" Offset="%2"/>', [color, c.offset])
        );
      }
    };
    ...
    LinearCanvasMega_.prototype.createBrush_ = function (ctx) {
      var brushObj = create(
        ctx,
        '<LinearMegaBrush MappingMode="Absolute" ' +
    ...
    CanvasMega_.prototype.createBrush_ = function (ctx) {
      ...
      var brushObj = create(
        ctx,
        '<RadialMegaBrush MappingMode="Absolute" ' +
      ...
      if (minRadius > 0) {
        ...
        for (var i = 0, c; (c = colors[i]); i++) {
          c.offset =
            minRadius / radius + ((radius - minRadius) / radius) * c.offset;
        }
      }
